...
class OrderApiTest extends TestCase
{
    ...
    /** @test */
    public function checkout_without_coupon_accepts_explicit_currency_and_safely_defaults_missing_currency()
    {
        $customerCart = Cart::create(['user_id' => $this->customer->id]);
        $customerCart->items()->create(['variant_id' => $this->variant1->id, 'quantity' => 1]);

        $this->actingAs($this->customer, 'sanctum')->postJson('/api/orders', [
            'shipping_address' => [
                'street' => '1 Kathmandu Road',
                'city' => 'Kathmandu',
                'state' => 'Bagmati',
                'zip' => '44600',
                'country' => 'Nepal',
        ...
        $wholesalerCart = Cart::create(['user_id' => $this->wholesaler->id]);
        $wholesalerCart->items()->create(['variant_id' => $this->variant1->id, 'quantity' => 1]);

        $this->actingAs($this->wholesaler, 'sanctum')->postJson('/api/orders', [
            'shipping_address' => [
                'street' => '2 International Road',
                'city' => 'New York',
                'state' => 'NY',
                'zip' => '10001',
                'country' => 'USA',
    ...
    /** @test */
    public function checkout_rejects_invalid_currency_before_creating_an_order()
    {
        ...
        $this->actingAs($this->customer, 'sanctum')->postJson('/api/orders', [
            'shipping_address' => [
                'street' => '1 Invalid Currency Road',
                'city' => 'Kathmandu',
                'state' => 'Bagmati',
                'zip' => '44600',
                'country' => 'Nepal',
    ...
    {
        // 1. Create active fixed discount of $5 on product
        Discount::create([
