...
function BannerCard({
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`relative overflow-hidden rounded-lg group cursor-pointer bg-gray-100 ${className}`}
    >
      <Image
        ...
        unoptimized // avoids Next.js domain restrictions for external URLs
      />
...
export default function MiddleSectionOffers({ offers }: Props) {
  ...
  // Debug in browser console — remove once working
  ...
  const [large, smallA, smallB, tall] = midOffers;

  return (
    <section className="max-w-7xl mx-auto px-4 md:px-10 py-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Left 2/3: large on top + two smalls on bottom */}
        <div className="md:col-span-2 flex flex-col gap-4">
          {large && <BannerCard offer={large} className="h-[200px] md:h-[220px]" index={0} />}
          <div className="grid grid-cols-2 gap-4">
            {smallA && <BannerCard offer={smallA} className="h-[150px] md:h-[170px]" index={1} />}
            {smallB && <BannerCard offer={smallB} className="h-[150px] md:h-[170px]" index={2} />}
          </div>
        </div>

        {/* Right 1/3: tall banner full height */}
        {tall && (
          <BannerCard offer={tall} className="h-[300px] md:h-full min-h-[395px]" index={3} />
        )}
      </div>
    </section>
  );
